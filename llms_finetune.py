...
def fix_text(example):
    if isinstance(example["text"], list):
        example["text"] = " ".join(example["text"])  # 或 example["text"][0]，根据你的需求
    return example
...
if __name__ == "__main__":
    ...
    parser.add_argument("-d", "--dataset_name", type=str, default="wikitext-2-raw-v1", 
                       help="Dataset name: wikitext-2-raw-v1, trivia_qa, squad, etc.")
    ...
    parser.add_argument("--cache_path", type=str, default="/largedisk/ragmia_models")
    ...
    parser.add_argument("--llm_judge", action="store_true", default=False)
    parser.add_argument("--llm_hamp", action="store_true", default=False)
    ...
    '''
    logger.info("Flash attention is disabled entirely by code modification.")
    ...
    if args.dataset_name == "squad":
        logger.info("SQuAD dataset: Question-Answering dataset with context")
    elif args.dataset_name == "trivia_qa":
        logger.info("TriviaQA dataset: Open-domain question answering")
    
    with accelerator.main_process_first():
        train_dataset, valid_dataset = dataset_prepare(args, tokenizer=tokenizer)
        if args.refer:
            train_dataset = None
            refer_data_path = f"{args.cache_path}/{args.dataset_name}/{args.dataset_config_name}/refer@{args.model_name}/rag_{args.rag}/llm_judge_{args.llm_judge}/llm_hamp_{args.llm_hamp}/"
            print("-------------refer_data_path---------------", refer_data_path)
            train_dataset = load_from_disk(refer_data_path)

        if args.dataset_name == "squad":

            if args.train_end_idx == 6000:  
                args.train_end_idx = len(train_dataset) 
            if args.eval_end_idx == 600: 
                args.eval_end_idx = len(valid_dataset)  
            logger.info(f"SQuAD dataset size - Train: {len(train_dataset)}, Valid: {len(valid_dataset)}")
            logger.info(f"Using train range: {args.train_sta_idx}:{args.train_end_idx}")
            logger.info(f"Using eval range: {args.eval_sta_idx}:{args.eval_end_idx}")
        
   
        if args.train_end_idx >= len(train_dataset):
    
            logger.info(f"Using full training dataset: {len(train_dataset)} samples")
        else:
            train_dataset = train_dataset.select(range(args.train_sta_idx, args.train_end_idx))
            logger.info(f"Using partial training dataset: {len(train_dataset)} samples")
            
        if args.eval_end_idx >= len(valid_dataset):
   
            logger.info(f"Using full validation dataset: {len(valid_dataset)} samples")
        else:
            valid_dataset = valid_dataset.select(range(args.eval_sta_idx, args.eval_end_idx))
            logger.info(f"Using partial validation dataset: {len(valid_dataset)} samples")
        train_dataset = train_dataset.map(fix_text)
        valid_dataset = valid_dataset.map(fix_text)
